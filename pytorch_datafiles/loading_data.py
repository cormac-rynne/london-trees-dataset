...
def loading_data():
    ...
    # Training
    train_path = os.path.join(cfg_data.DATA_PATH, 'train_data')
    logger.info(f'paths: data: {cfg_data.DATA_PATH}, train_path: {train_path}')

    train_set = London(
        cfg_data.TRAIN_JSON_FILEPATH,
        mode='train',
    ...
    # Validation
    test_path = os.path.join(cfg_data.DATA_PATH, 'test_data')

    val_set = London(
        cfg_data.TEST_JSON_FILEPATH,
        main_transform=None,
        img_transform=img_transform,
        gt_transform=gt_transform
    )
